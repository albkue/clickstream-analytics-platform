...
from ..config import FUNNEL_STEPS, Settings
from .models import Model, discover_models, compile_sql, select_models
...
def _build_incremental(
    ...
    Chosen over a plain append because the staging model re-reads a short
    overlap window on every run (late-arriving rows), so the same event can
...
def run(
    ...
    *,
    ...
    # Values available to models as {{ var('...') }}. The funnel definition
    # is passed in rather than hardcoded in SQL so config.FUNNEL_STEPS stays
    # the single place the step order is declared.
    variables: dict[str, Any] = {
        "session_timeout_minutes": settings.session_timeout_minutes,
        "funnel_steps": ",".join(FUNNEL_STEPS),
    }
